 ...
 * @since       2.0
 */
class ShldapModelHost extends AdminModel
{
	/**
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_shldap.edit.host.data', array());
	...
	public function setDefault($pk)
	{
		...
		if ($this->canEditState($table) && $table->load($pk) && $this->checkout($pk))
		{
			...
			Table::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_shconfig/tables');

			$configTable = Table::getInstance('Item', 'ShconfigTable', array());
