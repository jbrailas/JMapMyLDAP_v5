...
class SHUserAdaptersLdap implements SHUserAdapter
{
	...
	public function __construct(array $credentials, $config = null, array $options = array())
	{
		...
		if ($this->isNew = SHUtilArrayhelper::getValue($options, 'isNew', false, 'boolean'))
		{
			...
			{
				throw new RuntimeException(Text::sprintf('LIB_SHUSERADAPTERSLDAP_ERR_10909', $this->username), 10909);
			}
		...
		if ($this->_usePlugins)
		{
			...
			*/
			if (Version::MAJOR_VERSION < 4) {
				...
				//$extras = SHFactory::getDispatcher('ldap')->triggerEvent(
				//	'onLdapBeforeRead', 
				//);
				//$extras = array();
				$extras = Factory::getApplication()->triggerEvent('onLdapBeforeRead', array(&$this, array('dn' => $this->_dn, 'source' => __METHOD__)));
				//new joomla 5 way
				//$dispatcher = Factory::getApplication()->getDispatcher();
				//$event = new Event('onLdapBeforeRead', array(&$this, array('dn' => $this->_dn, 'source' => __METHOD__)));
				//$extras = $dispatcher->dispatch('onLdapBeforeRead', $event);
			}
			...
			$needToFind[] = $this->client->ldap_userAccountControl;
			...
			) === false)
			{
				...
				throw new RuntimeException(Text::_('LIB_SHUSERADAPTERSLDAP_ERR_10912'), 10912);
			}
	...
	/**
	 * Return the users enabled-disabled (userAccountControl) attribute from Ldap.
	 *
	 * @param   boolean  $key      If true returns the key of the full name instead of value.
	 * @param   mixed    $default  The default value.
	 *
	 ...
	 * @since   2.1
	 */	
	public function getEnabledDisabled($key = false, $default = null) {
		...
		if ($key)
		{
			...
			return $this->client->keyUserAccountControl;
		}

		// Find the Ldap attribute name key
		if (!is_null($this->client->keyUserAccountControl))
			$key = $this->client->keyUserAccountControl;
		else
			$key = $this->client->ldap_userAccountControl;
		
		//print_r($this->client);

		if ($value = $this->getAttributes($key)) {
			if (isset($value[$key][0])) {
				// ldap_userAccountControl found so lets return it
				return $value[$key][0];
			}
		}
	...
	public function setPassword($new, $old = null, $authenticate = false)
	{
		...
		if ($authenticate)
		{
			if (empty($old))
			{
				throw new InvalidArgumentException(Text::_('LIB_SHUSERADAPTERSLDAP_ERR_10917', 10917));
			}

			if (!$this->client->bind($this->_dn, $old))
			{
				// Incorrect old password
				throw new InvalidArgumentException(Text::_('LIB_SHUSERADAPTERSLDAP_ERR_10918', 10918));
			}
		}
	...
	private function _genPassword($password)
	{
		...
		else
		{
			...
			return UserHelper::getCryptedPassword(
	...
	public function commitChanges($options = array())
	{
		...
		if (!$userWrite && $this->client->bindStatus !== SHLdap::AUTH_PROXY)
		{
			if (!$this->client->proxyBind())
			{
				...
				throw new RuntimeException(Text::_('LIB_SHUSERADAPTERSLDAP_ERR_10901'), 10901);
			}
		}
		...
		if (isset($this->_changes['dn']) && ($this->_changes['dn'] != $this->_dn))
		{
			...
			throw new InvalidArgumentException(Text::_('LIB_SHUSERADAPTERSLDAP_ERR_10922'), 10922);
		}
	...
	public function create($options = array())
	{
		...
		if (isset($this->_changes['block']) && !is_array($this->_changes['block']))
		{
			$block = $this->_changes['block'];
			unset($this->_changes['block']);
			$this->_changes[$this->getEnabledDisabled(true)] = array($block);
		}		
	...
	public function delete($options = array())
	{
		...
		$this->_dn = new RuntimeException(Text::_('LIB_SHUSERADAPTERSLDAP_ERR_10906'), 10906);
