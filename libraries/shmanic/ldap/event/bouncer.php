 ...
 * @since       2.0
 */
class SHLdapEventBouncer extends Event
{
	/**
	...
	public function onUserBeforeSave($user, $isNew, $new)
	{
		...
		if ($isLdapExisting)
		{
			...
			if (SHLdapHelper::triggerEvent('onSHUserBeforeSave', array($user, $isNew, $new)) !== false)
			{
				try
				{
					// Commit the changes to the Adapter if present
					SHLdapHelper::commitChanges($adapter, true, true);
					SHLog::add(Text::sprintf('LIB_SHLDAPEVENTBOUNCER_DEBUG_10986', $username), 10986, Log::DEBUG, 'ldap');
				}
				catch (Excpetion $e)
				{
					SHLog::add($e, 10981, Log::ERROR, 'ldap');
				}
		...
		elseif ($isNew)
		{
			...
			//if (count($results = Factory::getApplication()->triggerEvent('askUserCreation')))
			{
				...
				{
					Factory::getSession()->set('created', $username, 'ldap');
	...
	public function onUserAfterSave($user, $isNew, $success, $msg)
	{
		if ($this->isLdap)
		{
			...
			Factory::getSession()->clear('created', 'ldap');
		}
	}
	...
	public function onUserLogin($user, $options = array())
	{
		...
		catch (Exception $e)
		{
			...
			SHLog::add($e, 10991, Log::ERROR, 'ldap');
		...
		//$UserLoginEvent = new Event('onUserLogin', array(&$instance, $options));
		//$result = $dispatcher->dispatch('onUserLogin', $UserLoginEvent);

		if ($result === false)
		{
			// Due to Joomla's inbuilt User Plugin, we have to raise an exception to abort login
			throw new RuntimeException(Text::sprintf('LIB_SHLDAPEVENTBOUNCER_ERR_10999', $user['username']), 10999);
		}

		// Check if any changes were made that need to be saved
		if ($result === true || isset($options['change']))
		{
			SHLog::add(Text::sprintf('LIB_SHLDAPEVENTBOUNCER_DEBUG_10984', $user['username']), 10984, Log::DEBUG, 'ldap');
			...
			try
			{
				...
				{
					SHLog::add(Text::sprintf('LIB_SHLDAPEVENTBOUNCER_ERR_10988', $user['username']), 10988, Log::ERROR, 'ldap');
				}
			}
			catch (Exception $e)
			{
				SHLog::add($e, 10989, Log::ERROR, 'ldap');
			}
		}
	...
	public function onUserLogout($user, $options = array()) {
		if ($this->isLdap) {
			...
			//$dispatcher = Factory::getApplication()->getDispatcher();
			///$UserLoginEvent = new Event('onUserLogin', array(&$instance, $options));
			///return $dispatcher->dispatch('onUserLogin', $UserLoginEvent);
