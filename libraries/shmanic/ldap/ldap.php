...
class SHLdap
{
	...
	/**
	 * Enabled-Disabled attribute (userAccountControl)
	 *
	 * @var    string
	 * @since  2.1
	 */
	protected $ldap_userAccountControl = null;	
	 ...
	 */
	public static function getInstance($id = null, array $authorised = array(), Registry $registry = null)
	{
		...
		if (count($errors) > 1)
		{
			...
			throw new SHExceptionStacked(Text::_('LIB_SHLDAP_ERR_10411'), 10411, $errors);
		}
		else
		{
	...
	public function __get($name)
	{
		switch ($name)
		{
			...
			case 'keyUserAccountControl':
				return $this->ldap_userAccountControl;
				break;
	...
	public function __construct($configObj = null)
	{
		...
		elseif ($configObj instanceof \Joomla\Registry\Registry)
		{
		...
		{
			// Unknown format
			throw new InvalidArgumentException(Text::_('LIB_SHLDAP_ERR_990'), 990);
		}
		...
		if (!extension_loaded('ldap'))
		{
			...
			throw new RunTimeException(Text::_('LIB_SHLDAP_ERR_991'), 991);
		}
	...
	public function addDebug($message)
	{
		...
		SHLog::add($message, 101, Log::DEBUG, 'ldap');
	...
	public function authenticate($authenticate = self::AUTH_NONE, $username = null, $password = null)
	{
		...
		else
		{
			...
			//if ($dn) 
				//$this->addDebug("user " . $username . " is not authenticated!");
			//else
				//$this->addDebug("user " . $username . " is authenticated successfully!");
		}
	...
	public function connect()
	{
		...
		if (empty($this->host))
		{
			throw new InvalidArgumentException(Text::_('LIB_SHLDAP_ERR_10001'), 10001);
		}

		// If there is a connection already, then close it before proceeding
		$this->close();

	$this->addDebug("Attempting connection to LDAP with host {$this->host}:{$this->port}");
		...
		if (!$this->isConnected())
		{
			...
			throw new SHLdapException(
				$this->getErrorCode(), 10002, Text::_('LIB_SHLDAP_ERR_10002')
			);
		}
		...
		if ($this->use_v3)
		{
			...
			if (!ldap_set_option($this->resource, LDAP_OPT_PROTOCOL_VERSION, 3))	
			{
				...
				throw new SHLdapException($this->getErrorCode(), 10003, Text::_('LIB_SHLDAP_ERR_10003'));
			}
		}
		...
		if (!ldap_set_option($this->resource, LDAP_OPT_REFERRALS, intval($this->use_referrals)))
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10004, Text::_('LIB_SHLDAP_ERR_10004'));
		}
		...
		if ($this->negotiate_tls)
		{
			...
			{
				// Failed to start TLS
				throw new SHLdapException($this->getErrorCode(), 10005, Text::_('LIB_SHLDAP_ERR_10005'));
			}
		}
	...
	{
		///return is_resource($this->resource); //changed in PHP 8.1
		//In PHP 8.1 the LDAP functions were changed to expect an [LDAP\Connection] instance now
		if ($this->resource !== false && !is_null($this->resource))
			$is_connected = true;
		else
			$is_connected = false;
		return $is_connected;
	...
	protected function operationAllowed()
	{
		if (!$this->isConnected())
		{
			...
			throw new RuntimeException(Text::_('LIB_SHLDAP_ERR_10006'), 10006);
		}

		if ($this->bind_status === self::AUTH_NONE)
		{
			// There is no binded user
			throw new RuntimeException(Text::_('LIB_SHLDAP_ERR_10007'), 10007);
		}
	}
	...
	public function proxyBind()
	{
		...
		if (@ldap_bind($this->resource, $this->proxy_username, $password)) //changed for php 8.1
		//if (@ldap_bind($this->resource, $this->proxy_username, $password) !== false && !is_null(@ldap_bind($this->resource, $this->proxy_username, $password)))
		{
	...
	public function compare($dn, $attribute, $value)
	{
		...
		if ($result === -1)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10131, Text::_('LIB_SHLDAP_ERR_10131'));
		}
	...
	public function search($dn = null, $filter = null, $attributes = array())
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10102, Text::_('LIB_SHLDAP_ERR_10102'));
		}
	...
	public function read($dn, $filter = null, $attributes = array())
	{
		...
		{
			// An Ldap error has occurred
			throw new SHLdapException($this->getErrorCode(), 10112, Text::_('LIB_SHLDAP_ERR_10112'));
		}
	...
	{
		//if (!is_resource($result)) //changed in PHP 8.1
		//In PHP 8.1 the LDAP functions were changed to expect an [LDAP\Connection] instance now
		if ($result === false || is_null($result))
		{
			// The result parameter must be a resource
			throw new InvalidArgumentException(Text::_('LIB_SHLDAP_ERR_10121'), 10121);
		}
	...
	public function modify($dn, $attributes)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10141, Text::_('LIB_SHLDAP_ERR_10141'));
		}
	...
	public function addAttributes($dn, $attributes)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10171, Text::_('LIB_SHLDAP_ERR_10171'));
		}
	...
	public function deleteAttributes($dn, $attributes)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10161, Text::_('LIB_SHLDAP_ERR_10161'));
		}
	...
	public function replaceAttributes($dn, $attributes)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10151, Text::_('LIB_SHLDAP_ERR_10151'));
		}
	...
	public function add($dn, $attributes)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10191, Text::_('LIB_SHLDAP_ERR_10191'));
		}
	...
	public function delete($dn)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10181, Text::_('LIB_SHLDAP_ERR_10181'));
		}
	...
	public function rename($dn, $newRdn, $newParent = null, $deleteOldRdn = true)
	{
		...
		if ($result === false)
		{
			...
			throw new SHLdapException($this->getErrorCode(), 10201, Text::_('LIB_SHLDAP_ERR_10201'));
		}
	...
	public function getUserDn($username = null, $password = null, $authenticate = false)
	{
		if (empty($this->user_qry))
		{
			...
			throw new InvalidArgumentException(Text::_('LIB_SHLDAP_ERR_10301'), 10301);
		}
		...
		if (empty($DNs))
		{
			 ...
			 */
			$this->addDebug(Text::_('LIB_SHLDAP_ERR_10302'));
			throw new SHExceptionInvaliduser(Text::_('LIB_SHLDAP_ERR_10302'), 10302, $username);
		}
		...
		if ($authenticate)
		{
			...
			if ($useSearch)
			{
				...
				throw new SHExceptionInvaliduser(Text::_('LIB_SHLDAP_ERR_10303'), 10303, $username);
			}
			else
			{
				// Unable to bind directly to the given distinguished name parameters
				throw new SHExceptionInvaliduser(Text::_('LIB_SHLDAP_ERR_10304'), 10304, $username);
			}
		}
		else
		{
			...
			else
			{
				...
				if ($this->proxyBind())
				{
					...
					if ($result === false)
					{
						...
						throw new SHExceptionInvaliduser(Text::_('LIB_SHLDAP_ERR_10305'), 10305, $username);
					}
				}
				else
				{
	...
	public function getUserDnBySearch($username)
	{
		...
		if (empty($this->base_dn))
		{
			...
			throw new InvalidArgumentException(Text::_('LIB_SHLDAP_ERR_10321'), 10321);
		}
		...
		if (!$this->proxyBind())
		{
			...
			throw new InvalidArgumentException(Text::_('LIB_SHLDAP_ERR_10322'), 10322);
		}
